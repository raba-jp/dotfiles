...
if [ "$CODESPACES" = "true" ]; then
	...
	nix build --no-link --extra-experimental-features flakes --extra-experimental-features nix-command '.#homeConfigurations.codespace.activationPackage'
	"$(nix path-info --extra-experimental-features flakes --extra-experimental-features nix-command '.#homeConfigurations.codespace.activationPackage')"/activate
else
